...
export default function MountainClimberTestPage() {
  const router = useRouter();
  // Table data
  const table = [
    { age: '10 - 12', ranges: [
      { label: 'Average', min: 0, max: 20, color: 'yellow' },
      { label: 'Very Good', min: 21, max: 30, color: 'green' },
      { label: 'Excellent', min: 31, max: 40, color: 'green' },
      { label: 'Elite', min: 41, max: Infinity, color: 'blue' },
    ] },
    { age: '13 - 14', ranges: [
      { label: 'Average', min: 0, max: 30, color: 'yellow' },
      { label: 'Very Good', min: 31, max: 40, color: 'green' },
      { label: 'Excellent', min: 41, max: 50, color: 'green' },
      { label: 'Elite', min: 51, max: Infinity, color: 'blue' },
    ] },
    { age: '15 - 17', ranges: [
      { label: 'Average', min: 0, max: 40, color: 'yellow' },
      { label: 'Very Good', min: 41, max: 50, color: 'green' },
      { label: 'Excellent', min: 51, max: 60, color: 'green' },
      { label: 'Elite', min: 61, max: Infinity, color: 'blue' },
    ] },
    { age: '18 - 20', ranges: [
      { label: 'Average', min: 0, max: 50, color: 'yellow' },
      { label: 'Very Good', min: 51, max: 60, color: 'green' },
      { label: 'Excellent', min: 61, max: 70, color: 'green' },
      { label: 'Elite', min: 71, max: Infinity, color: 'blue' },
    ] },
    { age: '21 - 25', ranges: [
      { label: 'Average', min: 0, max: 60, color: 'yellow' },
      { label: 'Very Good', min: 61, max: 70, color: 'green' },
      { label: 'Excellent', min: 71, max: 80, color: 'green' },
      { label: 'Elite', min: 81, max: Infinity, color: 'blue' },
    ] },
    { age: '26 - 30', ranges: [
      { label: 'Average', min: 0, max: 50, color: 'yellow' },
      { label: 'Very Good', min: 51, max: 60, color: 'green' },
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('mountainClimberTestUploads');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 0, height: '100vh', overflowY: 'auto' }}>
      ...
      {/* Info icon */}
      <div style={{ position: 'absolute', top: 24, right: 18, zIndex: 10, cursor: 'pointer' }} onClick={() => router.push('/challanges/mountain-climber-test/info')}>
        <span style={{ color: 'black', fontSize: 32, fontWeight: 700 }}>ⓘ</span>
      </div>
      {/* Top image */}
      <div style={{ marginTop: 48, marginBottom: 8 }}>
        <Image src="/player5.png" alt="Mountain Climber Test" width={120} height={160} style={{ objectFit: 'contain', filter: 'grayscale(1)' }} />
      </div>
      {/* Title */}
      <div style={{ fontWeight: 500, fontSize: 36, color: '#222', fontFamily: 'serif', marginBottom: 8, textAlign: 'center' }}>Mountain Climber Test</div>
      {/* Video icon */}
      <div style={{ marginBottom: 16, cursor: 'pointer' }} onClick={() => router.push('/challanges/mountain-climber-test/upload')}>
