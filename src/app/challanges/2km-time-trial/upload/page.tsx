...
export default function TwoKmTimeTrialUploadPage() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      localStorage.setItem('twoKmTimeTrialUploads', JSON.stringify(uploads));
    }
  }, [uploads]);
  ...
  function handleAdd(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (file && time && age !== null) {
      ...
      setUploads([
        ...uploads,
        { label: `2km Time Trial at ${age}`, time: seconds, video: URL.createObjectURL(file), age },
      ]);
      setShowForm(false);
      setFile(null);
      setTime('');
  ...
  function handleEditSave(idx: number) {
    if (editTime) {
      let seconds = parseFloat(editTime);
      if (editTime.includes(':')) {
        const [mm, ss] = editTime.split(':').map(Number);
        seconds = mm * 60 + ss;
      }
      setUploads(uploads.map((u: typeof uploads[0], i: number) => i === idx ? { ...u, time: seconds } : u));
      setEditIndex(null);
      setEditTime('');
    }
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start', padding: 24 }}>
      ...
      <h2 style={{ fontFamily: 'serif', fontWeight: 700, fontSize: 32, marginBottom: 24, color: 'black' }}>2km Time Trial Results</h2>
      ...
      {showForm && (
        <form onSubmit={handleAdd} style={{ width: '100%', maxWidth: 400, display: 'flex', flexDirection: 'column', gap: 18, alignItems: 'center', background: '#f5f6fa', borderRadius: 16, padding: 24, marginBottom: 24 }}>
          ...
          <label style={{ fontSize: 18, fontFamily: 'serif', color: '#222' }}>
            ...
            <input type="text" value={time} onChange={e => setTime(e.target.value)} placeholder="e.g. 7:45 or 465" style={{ display: 'block', marginTop: 8, width: 120, fontSize: 18, padding: 4, borderRadius: 6, border: '1px solid #bbb' }} />
          </label>
          <label style={{ fontSize: 18, fontFamily: 'serif', color: '#222' }}>
            Age
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
              ...
              {editIndex === i ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <input type="text" value={editTime} onChange={e => setEditTime(e.target.value)} style={{ width: 80, fontSize: 16, padding: 4, borderRadius: 6, border: '1px solid #bbb', color: 'black' }} />
                ...
                <div style={{ fontSize: 18, fontFamily: 'serif', color: '#222' }}>Time: <b>{formatTime(u.time)}</b></div>
              )}
