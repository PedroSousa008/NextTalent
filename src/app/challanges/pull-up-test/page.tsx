...
export default function PullUpTestPage() {
  const router = useRouter();
  // Table data
  const table = [
    { age: '10 - 12', ranges: [
      { label: 'Average', min: 0, max: 2, color: 'yellow' },
      { label: 'Very Good', min: 3, max: 5, color: 'green' },
      { label: 'Excellent', min: 6, max: 8, color: 'green' },
      { label: 'Elite', min: 9, max: Infinity, color: 'blue' },
    ] },
    { age: '13 - 14', ranges: [
      { label: 'Average', min: 0, max: 3, color: 'yellow' },
      { label: 'Very Good', min: 4, max: 6, color: 'green' },
      { label: 'Excellent', min: 7, max: 9, color: 'green' },
      { label: 'Elite', min: 10, max: Infinity, color: 'blue' },
    ] },
    { age: '15 - 17', ranges: [
      { label: 'Average', min: 0, max: 4, color: 'yellow' },
      { label: 'Very Good', min: 5, max: 7, color: 'green' },
      { label: 'Excellent', min: 8, max: 10, color: 'green' },
      { label: 'Elite', min: 11, max: Infinity, color: 'blue' },
    ] },
    { age: '18 - 20', ranges: [
      { label: 'Average', min: 0, max: 5, color: 'yellow' },
      { label: 'Very Good', min: 6, max: 8, color: 'green' },
      { label: 'Excellent', min: 9, max: 11, color: 'green' },
      { label: 'Elite', min: 12, max: Infinity, color: 'blue' },
    ] },
    { age: '21 - 25', ranges: [
      { label: 'Average', min: 0, max: 6, color: 'yellow' },
      { label: 'Very Good', min: 7, max: 9, color: 'green' },
      { label: 'Excellent', min: 10, max: 12, color: 'green' },
      { label: 'Elite', min: 13, max: Infinity, color: 'blue' },
    ] },
    { age: '26 - 30', ranges: [
      { label: 'Average', min: 0, max: 5, color: 'yellow' },
      { label: 'Very Good', min: 6, max: 8, color: 'green' },
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('pullUpTestUploads');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 0, height: '100vh', overflowY: 'auto' }}>
      ...
      {/* Info icon */}
      <div style={{ position: 'absolute', top: 24, right: 18, zIndex: 10, cursor: 'pointer' }} onClick={() => router.push('/challanges/pull-up-test/info')}>
        <span style={{ color: 'black', fontSize: 32, fontWeight: 700 }}>ⓘ</span>
      </div>
      {/* Top image */}
      <div style={{ marginTop: 48, marginBottom: 8 }}>
        <Image src="/player5.png" alt="Pull-up Test" width={120} height={160} style={{ objectFit: 'contain', filter: 'grayscale(1)' }} />
      </div>
      {/* Title */}
      <div style={{ fontWeight: 500, fontSize: 36, color: '#222', fontFamily: 'serif', marginBottom: 8, textAlign: 'center' }}>Pull-up Test</div>
      {/* Video icon */}
      <div style={{ marginBottom: 16, cursor: 'pointer' }} onClick={() => router.push('/challanges/pull-up-test/upload')}>
