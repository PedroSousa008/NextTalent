...
export default function SitAndReachTestPage() {
  ...
  const table = [
    ...
    { age: '13 - 14', ranges: [
      { label: 'Average', min: 0, max: 18, color: 'yellow' },
      { label: 'Very Good', min: 19, max: 23, color: 'green' },
      { label: 'Excellent', min: 24, max: 28, color: 'green' },
      { label: 'Elite', min: 29, max: Infinity, color: 'blue' },
    ] },
    { age: '15 - 17', ranges: [
    ...
    { age: '18 - 20', ranges: [
      { label: 'Average', min: 0, max: 22, color: 'yellow' },
      { label: 'Very Good', min: 23, max: 27, color: 'green' },
      { label: 'Excellent', min: 28, max: 32, color: 'green' },
      { label: 'Elite', min: 33, max: Infinity, color: 'blue' },
    ] },
    { age: '21 - 25', ranges: [
      { label: 'Average', min: 0, max: 23, color: 'yellow' },
      { label: 'Very Good', min: 24, max: 28, color: 'green' },
      { label: 'Excellent', min: 29, max: 33, color: 'green' },
      { label: 'Elite', min: 34, max: Infinity, color: 'blue' },
    ] },
    { age: '26 - 30', ranges: [
      { label: 'Average', min: 0, max: 22, color: 'yellow' },
      { label: 'Very Good', min: 23, max: 27, color: 'green' },
    ...
    { age: '31 - 35', ranges: [
      { label: 'Average', min: 0, max: 21, color: 'yellow' },
      { label: 'Very Good', min: 22, max: 26, color: 'green' },
      { label: 'Excellent', min: 27, max: 31, color: 'green' },
      { label: 'Elite', min: 32, max: Infinity, color: 'blue' },
    ] },
    { age: '36+', ranges: [
      { label: 'Average', min: 0, max: 20, color: 'yellow' },
      { label: 'Very Good', min: 21, max: 25, color: 'green' },
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('sitAndReachTestUploads');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 0, height: '100vh', overflowY: 'auto' }}>
      ...
      {/* Info icon */}
      <div style={{ position: 'absolute', top: 24, right: 18, zIndex: 10, cursor: 'pointer' }} onClick={() => router.push('/challanges/sit-and-reach-test/info')}>
        <span style={{ color: 'black', fontSize: 32, fontWeight: 700 }}>ⓘ</span>
      </div>
      {/* Top image */}
      <div style={{ marginTop: 48, marginBottom: 8 }}>
        <Image src="/player5.png" alt="Sit and Reach Test" width={120} height={160} style={{ objectFit: 'contain', filter: 'grayscale(1)' }} />
      </div>
      {/* Title */}
      <div style={{ fontWeight: 500, fontSize: 36, color: '#222', fontFamily: 'serif', marginBottom: 8, textAlign: 'center' }}>Sit and Reach Test</div>
      {/* Video icon */}
      <div style={{ marginBottom: 16, cursor: 'pointer' }} onClick={() => router.push('/challanges/sit-and-reach-test/upload')}>
