...
export default function MedicineBallThrowTestPage() {
  const router = useRouter();
  // Table data
  const table = [
    { age: '10 - 12', ranges: [
      { label: 'Average', min: 0, max: 12.0, color: 'yellow' },
      { label: 'Very Good', min: 11.9, max: 11.0, color: 'green' },
      { label: 'Excellent', min: 10.9, max: 10.0, color: 'green' },
      { label: 'Elite', min: 9.9, max: 0, color: 'blue' },
    ] },
    { age: '13 - 14', ranges: [
      { label: 'Average', min: 0, max: 11.0, color: 'yellow' },
      { label: 'Very Good', min: 10.9, max: 10.0, color: 'green' },
      { label: 'Excellent', min: 9.9, max: 9.0, color: 'green' },
      { label: 'Elite', min: 8.9, max: 0, color: 'blue' },
    ] },
    { age: '15 - 17', ranges: [
      { label: 'Average', min: 0, max: 10.0, color: 'yellow' },
      { label: 'Very Good', min: 9.9, max: 9.0, color: 'green' },
      { label: 'Excellent', min: 8.9, max: 8.0, color: 'green' },
      { label: 'Elite', min: 7.9, max: 0, color: 'blue' },
    ] },
    { age: '18 - 20', ranges: [
      { label: 'Average', min: 0, max: 9.0, color: 'yellow' },
      { label: 'Very Good', min: 8.9, max: 8.0, color: 'green' },
      { label: 'Excellent', min: 7.9, max: 7.0, color: 'green' },
      { label: 'Elite', min: 6.9, max: 0, color: 'blue' },
    ] },
    { age: '21 - 25', ranges: [
      { label: 'Average', min: 0, max: 8.0, color: 'yellow' },
      { label: 'Very Good', min: 7.9, max: 7.0, color: 'green' },
      { label: 'Excellent', min: 6.9, max: 6.0, color: 'green' },
      { label: 'Elite', min: 5.9, max: 0, color: 'blue' },
    ] },
    { age: '26 - 30', ranges: [
      { label: 'Average', min: 0, max: 9.0, color: 'yellow' },
      { label: 'Very Good', min: 8.9, max: 8.0, color: 'green' },
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('medicineBallThrowTestUploads');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 0, height: '100vh', overflowY: 'auto' }}>
      ...
      {/* Info icon */}
      <div style={{ position: 'absolute', top: 24, right: 18, zIndex: 10, cursor: 'pointer' }} onClick={() => router.push('/challanges/medicine-ball-throw-test/info')}>
        <span style={{ color: 'black', fontSize: 32, fontWeight: 700 }}>ⓘ</span>
      </div>
      {/* Top image */}
      <div style={{ marginTop: 48, marginBottom: 8 }}>
        <Image src="/player5.png" alt="Medicine Ball Throw Test" width={120} height={160} style={{ objectFit: 'contain', filter: 'grayscale(1)' }} />
      </div>
      {/* Title */}
      <div style={{ fontWeight: 500, fontSize: 36, color: '#222', fontFamily: 'serif', marginBottom: 8, textAlign: 'center' }}>Medicine Ball Throw Test</div>
      {/* Video icon */}
      <div style={{ marginBottom: 16, cursor: 'pointer' }} onClick={() => router.push('/challanges/medicine-ball-throw-test/upload')}>
