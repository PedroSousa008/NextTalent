...
export default function WallSitTestPage() {
  const router = useRouter();
  // Table data
  const table = [
    { age: '10 - 12', ranges: [
      { label: 'Average', min: 0, max: 30, color: 'yellow' },
      { label: 'Very Good', min: 31, max: 45, color: 'green' },
      { label: 'Excellent', min: 46, max: 60, color: 'green' },
      { label: 'Elite', min: 61, max: Infinity, color: 'blue' },
    ] },
    { age: '13 - 14', ranges: [
      { label: 'Average', min: 0, max: 45, color: 'yellow' },
      { label: 'Very Good', min: 46, max: 60, color: 'green' },
      { label: 'Excellent', min: 61, max: 90, color: 'green' },
      { label: 'Elite', min: 91, max: Infinity, color: 'blue' },
    ] },
    { age: '15 - 17', ranges: [
      { label: 'Average', min: 0, max: 60, color: 'yellow' },
      { label: 'Very Good', min: 61, max: 90, color: 'green' },
      { label: 'Excellent', min: 91, max: 120, color: 'green' },
      { label: 'Elite', min: 121, max: Infinity, color: 'blue' },
    ] },
    { age: '18 - 20', ranges: [
      { label: 'Average', min: 0, max: 90, color: 'yellow' },
      { label: 'Very Good', min: 91, max: 120, color: 'green' },
      { label: 'Excellent', min: 121, max: 150, color: 'green' },
      { label: 'Elite', min: 151, max: Infinity, color: 'blue' },
    ] },
    { age: '21 - 25', ranges: [
      { label: 'Average', min: 0, max: 120, color: 'yellow' },
      { label: 'Very Good', min: 121, max: 150, color: 'green' },
      { label: 'Excellent', min: 151, max: 180, color: 'green' },
      { label: 'Elite', min: 181, max: Infinity, color: 'blue' },
    ] },
    { age: '26 - 30', ranges: [
      { label: 'Average', min: 0, max: 90, color: 'yellow' },
      { label: 'Very Good', min: 91, max: 120, color: 'green' },
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('wallSitTestUploads');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: 0, height: '100vh', overflowY: 'auto' }}>
      ...
      {/* Info icon */}
      <div style={{ position: 'absolute', top: 24, right: 18, zIndex: 10, cursor: 'pointer' }} onClick={() => router.push('/challanges/wall-sit-test/info')}>
        <span style={{ color: 'black', fontSize: 32, fontWeight: 700 }}>ⓘ</span>
      </div>
      {/* Top image */}
      <div style={{ marginTop: 48, marginBottom: 8 }}>
        <Image src="/player5.png" alt="Wall Sit Test" width={120} height={160} style={{ objectFit: 'contain', filter: 'grayscale(1)' }} />
      </div>
      {/* Title */}
      <div style={{ fontWeight: 500, fontSize: 36, color: '#222', fontFamily: 'serif', marginBottom: 8, textAlign: 'center' }}>Wall Sit Test</div>
      {/* Video icon */}
      <div style={{ marginBottom: 16, cursor: 'pointer' }} onClick={() => router.push('/challanges/wall-sit-test/upload')}>
